...
// 1. Gather all question texts to find duplicates
const textCounts = {};
for (const file of files) {
  if (!fs.existsSync(file)) continue;
  const content = fs.readFileSync(file, 'utf8');
  const textRegex = /questionText:\s*'(1[^']+)'/g;
  let match;
...
for (const file of files) {
  ...
  // Fix 3: Duplicate IDs
  content = content.replace(/id:\s*'(1[^']+)'/g, (match, id) => {
  ...
  // Fix 1: Answer mismatches
  content = content.replace(/(answerText:\s*')([^']+)('[\s\S]*?options:\s*\Z)([\s\S]*?)(\])/g, (match, p1, answer, p3, optionsStr, p5) => {
    let options = [];
    let currentOpt = '';
    let inQuote = false;
  ...
  // Fix 4: Repeated Question Texts
  content = content.replace(/questionText:\s*'([^']+)'/g, (match, text) => {
    let baseText = text.replace(/\s*\(variant \d+\)$/i, '');
